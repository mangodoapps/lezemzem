...
class ActivitiesController extends BaseController {
    ...
    static async getActivities(req, res) {
        ...
        const datas = await Activity.get();

        return view(res, "admin.activities.index", {
    ...
    static async getActivitiesNew(req, res) {
        const user = await Auth.guard("admin").user(req);
        const data = new Activity();
        data.creator = user.id;
        data.created_at = DateTime.getTime();
        await data.save();
        return res.redirect("/admin/activities/edit?id=" + data.id);
    }

    static async getActivitiesEdit(req, res) {
        ...
        const data = await Activity.where({ "id": req.query.id }).first();
        if (!data) {
            return res.redirect("/admin/activities");
        }
        ...
        return view(res, "admin.activities.form", {
            ...await BaseController.boot(req),
            data,
            languages,
        });
    ...
    static async getActivitiesDelete(req, res) {
        const validate = Validator.make(req.query, {
            "id": "required|integer",
        });
        if (validate) {
    ...
    static async postActivitiesEdit(req, res) {
        const user = await Auth.guard("admin").user(req);
        const data = await Activity.where({ "id": req.query.id }).first();
        ...
        const seo_check = await Activity.select("id").whereNotIn("id", [req.query.id]).where({ "seo_url": req.body.url }).get();
